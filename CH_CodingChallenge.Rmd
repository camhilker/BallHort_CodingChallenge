...
This analysis shows that ~20,000 genes are differentially expressed. This is a lot to consider, and I would assume that part of this difference in expression should be written off as natural biological variance and/or background noise. Now I will attempt to use additional methods in order to quantify the most highly differential genes.

Finding the "top 10" upregulated and downregulated genes for each replicate:
```{r}
...
I experimented with taking the log of the expression values rather than raw values, to see if it created a clearer heatmap.
```{r}
...
I decided to do this using a total of 50 genes (25 from both up- and down-regulated expression). I also decided to center and scale the expression values across the 6 samples in order to highlight the expression differences rather than the raw expression levels (as in the earlier heatmaps).
```{r}
...
heatmap.2(top50.ave.heatmap, main = "Overall Top 50 Genes, Clustered", labRow = exprs.data$Name[top50.ave], dendrogram = "row", margins = c(7,7), scale = "row", col = redgreen(100), cexCol = 1, cexRow = 0.2, trace = "none")
```